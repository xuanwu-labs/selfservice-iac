...
- [ ] 1.1 新增 `platform/` 顶层目录与 `platform/cmd/platform` 主二进制骨架（HTTP server 启停、健康检查、配置加载）
- [ ] 1.2 在 `platform/internal/adapters/{git,state,policy,cost,notify,cloud}` 定义可插拔适配器接口（见 docs/03 §2），各提供 noop/stub 默认实现
- [ ] 1.3 定义 `platform/internal/terramate` 的 `TerramateAdapter` 接口与 exec 默认实现（封装 `terramate run/generate` 子进程调用、退出码与 stdout/stderr 捕获）
- [ ] 1.4 增加 CI lint 规则：禁止 `platform/**` import Terramate 被保护目录的非公开包；`make build` 验证不破坏 Terramate 主仓库
- [ ] 1.5 测试：`go test ./platform/internal/adapters/... ./platform/internal/terramate/...`（接口契约 + exec 适配器用 fake terramate 脚本断言）
- [ ] 1.6 脚本：在 `scripts/初始化/` 增加平台配置加载与适配器装配辅助脚本骨架

## 02-元数据存储与迁移

- [ ] 2.1 选定元数据 DB（PostgreSQL），在 `platform/internal/store` 建立访问层与连接池
- [ ] 2.2 在 `platform/db/migrations/` 编写初始 schema 迁移，落地 `docs/04` 全部表（teams/projects/bundles/modules/catalog_items/stacks/requests/request_events/approvals/artifacts/workspaces/workspace_checkouts/drift_runs/drift_records/identities/role_bindings/audit_logs/adapters_config）
- [ ] 2.3 测试：`go test ./platform/internal/store/...`（CRUD + 业务唯一约束 + 迁移 up/down 幂等）
- [ ] 2.4 脚本：在 `scripts/迁移/` 产出 schema 初始化与迁移执行脚本

## 03-模块注册与服务目录

- [ ] 3.1 实现 `platform/internal/registry`：注册 Git 模块、版本管理、拉取并解析 `variables.tf` 生成 `variables_contract_json`
- [ ] 3.2 实现注册时校验：调用 `terraform validate`（可选 `init`），状态机 `pending-validation → validated | validation-failed`
- [ ] 3.3 实现 `platform/internal/catalog`：发布目录项、从契约裁剪 `form_schema_json`、维护 `defaults_json` 最佳实践覆盖、可见性控制
- [ ] 3.4 测试：`go test ./platform/internal/registry/... ./platform/internal/catalog/...`（用 fixture 模块断言契约提取、默认值注入、可见性过滤）
- [ ] 3.5 脚本：在 `scripts/模块注册/` 产出批量注册原子模块的辅助脚本

## 04-团队归属与分层模型（D24 stack 模型可配置化）

> **Phase 策略**：Phase 1 hard-code 三层（global/middleware/application + per-component 粒度），不上 D24 可配置/多粒度；Phase 2 开放 D24 path_template 自定义 + StackGranularity 多策略；Phase 3 开放 D26 版本化迁移。

- [ ] 4.1 实现 `platform/internal/tenancy`：团队/项目组/bundle CRUD（**bundle 可选**，bundle_id NULL 表示无 bundle）、资源归属固定规则（RDS→DBA 等）
- [ ] 4.2 Phase 1 实现固定三层 `platform/internal/stackmodel` seed（global/middleware/application），只暴露读取，不开放管理员改模板；Phase 2 再实现 Layer 规则集版本化引擎与 v1↔v2 diff viewer
- [ ] 4.3 实现 `PathGenerator`：按 D29 layer-first Path Contract 输出 `repo_path + state_key + stack_id + terramate_tags_json`；模板变量 env/tenant/team/bundle/component/layer/custom_kv；**codegen MUST 调用此组件，MUST NOT 字符串拼接**
- [ ] 4.4 实现 `StackGranularity` 评估器：per-component（默认）/per-bundle/per-team/custom；读 `stack_grouping_rules` + catalog 项 stack_grouping 字段
- [ ] 4.5 实现跨层依赖图（stack 间 depends-on，按 layer.depends_on 校验无环）
- [ ] 4.6 Phase 2 实现 **MigrationPlanner**（`platform/internal/stackmodel/migrator`）：输入 layer_rule_set 变更草案 → dry-run 对每个受影响 stack 重渲染 path 对比 → 输出 per-stack Tier 分类（1/2/3）+ 影响面报告 + rollback_token；UI 可视化 before/after path 树
- [ ] 4.7 Phase 3 才实现 **StateMover 半自动能力**（`platform/internal/stackmodel/statemover`）：默认 feature_flag=off；开启时必须双人审批 + state snapshot + `terraform plan -detailed-exitcode` exit 0；Phase 1/2 仅生成人工 SOP 和审计任务
...
- [ ] 4.12 测试：`go test ./platform/internal/tenancy/... ./platform/internal/stackmodel/...`（路径模板渲染、bundle 可选、归属判定、依赖图拓扑、自定义层增删、StackGranularity 评估、整体版本化、per-stack Tier 分类、Tier 2 state mv + plan=0 校验 + 失败回滚、逆向 mv、CMDB 同步）

## 05-代码生成（D25 模块零侵入 + cardinality 调用方注入）

- [ ] 5.1 实现 `platform/internal/codegen`：输入（表单值 + 模块契约[纯 scalar] + 默认值 + 依赖图）→ 输出（stack 目录树、`stack.tm.hcl`、`main.tf` 调模块、`backend.tf` 远程 state、跨层 `data` 注入）；**调用 PathGenerator 渲染路径**（不字符串拼接）
...
- [ ] 5.6 测试：`go test ./platform/internal/codegen/...`（golden file：固定输入 → 固定目录树与文件内容；覆盖 single/list/map 三种 cardinality + bundle 有/无 + 自定义 layer；**用社区模块 fixture 验证零侵入**）

## 06-Terramate 适配器与编排引擎

- [ ] 6.1 实现 `platform/internal/orchestrator` 工单状态机：`submitted → generating → planning → plan-ready → pending-approval → applying → reconciling → succeeded | failed-retryable | failed-terminal | waiting-manual | reconcile-pending | blocked-policy`；Phase 2 增加 `pending-admission`；每步持久化 `request_events` + 乐观锁 version
...
- [ ] 6.4 测试：`go test ./platform/internal/orchestrator/...`（状态机迁移、OPA 阻断、plan artifact TTL、apply 中断转 waiting-manual、reconcile-pending、幂等，用 fake terramate）

## 07-执行目录治理（workspace git）

- [ ] 7.1 实现 `platform/internal/workspace`：基于 go-git 的工作仓库 clone/fetch、`git worktree` 分配、commit/push
...
- [ ] 7.5 测试：`go test ./platform/internal/workspace/...`（用内存 fake repo 验证 clone/worktree/重启恢复/对账）
...
- [ ] 8.2 实现 `platform/cmd/drift-scheduler` + `platform/internal/drift`：调度器（分层时间窗 + 令牌桶限流）、worker（只读 plan + `terraform show -json` 解析差异）
- [ ] 8.3 实现漂移事件 `drift.detected` 与同步策略执行（adopt-cloud / restore-desired，走工单留审批）（对应 docs/13）
- [ ] 8.4 测试：`go test ./platform/internal/drift/...`（plan JSON 样本解析、限流不超上限、同步策略执行）
- [ ] 8.5 脚本：在 `scripts/状态同步/` 产出 state key 对账、漂移手动触发、同步策略执行辅助脚本

## 09-平台 API 与 RBAC 审批

- [ ] 9.1 实现 `platform/internal/api`：`/api/v1` 下 catalog/requests/approvals/drift/modules/audit 端点 + OpenAPI 描述
- [ ] 9.2 实现 `platform/internal/auth`：OIDC/token 鉴权 + RBAC（`role_bindings` 评估，按 team/project/bundle/stack/layer × action）
- [ ] 9.3 实现审批流（多级/串行/会签）与 `POST /requests` 同步建单 + 异步推进
- [ ] 9.4 实现 `platform/internal/events` + `audit`：事件总线、Webhook 分发、append-only 审计日志
- [ ] 9.5 测试：`go test ./platform/internal/api/... ./platform/internal/auth/...`（端点契约、越权 403、审批流转、审计写入）

## 10-前端、e2e 与整体验证

- [ ] 10.1 搭建 `frontend/`（React + 表单引擎，由 `form_schema_json` 声明式渲染），覆盖目录/申请/审批/漂移/历史视图
- [ ] 10.2 在 `e2etests/platform/`（新增）落端到端：表单提交 → 工单 → 生成代码 → 断言 stack 目录与 state key；漂移检测端到端
- [ ] 10.3 全量验证：`make build`（Terramate 主二进制不受影响）+ `make test`（Terramate 回归绿色）+ `go test ./platform/...`
- [ ] 10.4 文档：更新 AGENTS.md / README，说明 `platform/` 边界、运行方式、scripts 用法
...
- [ ] 11.2 **Phase 1**：实现单 OIDC issuer + `claim_mapping` 登录（`platform/internal/auth/oidc`），外部组只作为输入，最终权限仍由平台 RBAC 判定
...
- [ ] 12.6 测试：`go test ./platform/internal/approval/...`（各 mode、超时升级、驳回、RBAC 路由、双门禁时序、plan 产物输入）
...
- [ ] 13.4 实现 `platform/internal/executor`（`Executor` 接口 + 四模式实现：`ProcessExecutor` 节点预装工具链+cgroups+netns / `ContainerExecutor` 宿主 docker/podman / `K8sExecutor` Job/Pod / `RemoteNodeExecutor` SSH 节点池），统一契约（worktree 挂载、资源/网络限制、退出码、日志流式回传）；编排引擎与漂移检测均通过接口调用（D20）
- [ ] 13.5 实现 `platform/internal/executor/imagebuilder` + 可插拔 `BuildDriver`（docker/buildah/kaniko）+ `ImageRegistry` 适配器（Harbor/ACR/ECR/ACR/Docker Registry/内嵌本地/文件中转）：Web 保存工具链版本 → 渲染 Dockerfile → 异步构建（K8s 用 kaniko 无 daemon，进程用 buildah）→ tag → push → 灰度；process 模式不触发构建（D22）
- [ ] 13.6 实现 `platform/internal/installer`（process 模式按 Web 基线自动 reconcile 节点：mirror 下载→checksum→装+symlink+`toolchain_manifest`；离线手动投放兜底）
- [ ] 13.7 版本兼容校验三时机（注册/catalog/执行前 `validateCompatibility`，镜像解 manifest 或预装读 `toolchain_manifest`），不兼容阻断给可操作提示（D22）
- [ ] 13.8 测试：`go test ./platform/internal/executor/... ./platform/internal/executor/imagebuilder/... ./platform/internal/installer/...`（四模式契约一致、BuildDriver 三驱动、镜像构建 push、兼容校验阻断、installer reconcile）+ e2e（同一工单四模式退出码/日志一致；process 零镜像；k8s kaniko 构建；离线手动投放）
...
- [ ] 14.6 测试：`go test ./platform/internal/importer/...` + e2e（真实资源 import、不支持资源、敏感字段、批量）

## 15-扩展能力整体验证

- [ ] 15.1 全量验证：`make build` + `make test` + `go test ./platform/...`（含 11-14 新包）
...
- [ ] 18.1 **Phase 1**：实现 `platform/internal/cmdb` resource ingester（apply 后从 state JSON 解析 upsert `resources`）、tag 归一化、CMDB 查询 API；ingester 失败进入 `reconcile-pending`，不回滚 apply
...
- [ ] 19.1 实现 `platform/internal/cloudcreds`：云账号纳管（`cloud_accounts` CRUD + bootstrap 凭据 secret store 引用）
...
- [ ] 19.12 测试：`go test ./platform/internal/cloudcreds/...`（grant 过滤、OIDC 路径、占位符填充、日志脱敏、角色权限聚合、轮换窗口）
...
- [ ] 20.1 实现 `platform/internal/envtenant/envmanager`：Environment CRUD（env_logical_id UNIQUE + stage 治理强度 + cloud_account_id + region + tag_namespace_json + network_topology）；UI 入"环境管理"顶级导航
- [ ] 20.2 实现 `platform/internal/envtenant/tenantmanager`：Tenant CRUD（默认初始化 platform-default，isolation_level ∈ {vpc-per-env, account-per-env}，拒绝非法值含 shared-vpc）；UI 入"租户管理"顶级导航
...
- [ ] 20.10 测试：`go test ./platform/internal/envtenant/...`（三元组 UNIQUE 校验、vpc_stack outputs 校验、account-per-env override 校验、BindingResolver 查询、env_scope 过滤、freeze/deprecate lifecycle）
...
- [ ] 21.1 实现 `platform/internal/codegen/paramresolver`：9 阶段管道主循环（S1-S9），每阶段含 source 加载 + rank 覆盖 + 校验门
...
- [ ] 21.6 实现 `platform/internal/codegen/tagmerger`：Tag 7 层合并（L1-L7）+ L1 永远赢 + L2-L6 后层覆盖前层 + L7 白名单约束（catalog.user_allowed_tag_keys_json）
...
- [ ] 21.8 实现 `platform/internal/codegen/provenance`：ProvenanceWriter 写 requests.resolved_params_json，每变量含 {value, source, rank, overridden_from/rule_id/env_id/tenant_id/binding_id/user_attempted_override/user_attempted_value}；tags 字段含 layers_applied + user_overrides_blocked + layers_merged_summary
...
- [ ] 21.14 测试：`go test ./platform/internal/codegen/paramresolver/... ./platform/internal/codegen/tagmerger/... ./platform/internal/codegen/provenance/...`（9 阶段顺序、rank 优先级、L1 不可覆盖、L7 白名单拒绝、治理类 user_attempted_override 留痕、云 tag 约束校验、provenance 完整性）
...
- [ ] 22.7 测试：`go test ./platform/internal/security/...`（break-glass 凭据验证+录屏+retroactive 超时、sensitive stripper 覆盖、OPA IAM 聚合校验规则、STS 不缓存断言）
