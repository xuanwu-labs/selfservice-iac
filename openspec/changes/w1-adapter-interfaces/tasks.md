...
- [ ] 1.2 Create `server/core/adapters/cloud/cloud.go`: CloudProvider interface (ValidateCredentials/ListRegions) + NoopCloud + CloudCredentials struct
- [ ] 1.3 Create `server/core/adapters/state/state.go`: StateBackend interface (Read/Write/Delete/Lock/Unlock) + NoopState
- [ ] 1.4 Create `server/core/adapters/policy/policy.go`: PolicyEngine interface (Evaluate) + NoopPolicy + PolicyResult struct
- [ ] 1.5 Create `server/core/adapters/cost/cost.go`: CostEstimator interface (Estimate) + NoopCost + CostResult struct
...
- [ ] 2.1 Create `server/core/terramate/terramate.go`: TerramateAdapter interface (Run/Version) + RunResult struct
